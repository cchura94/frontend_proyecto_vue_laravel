<template>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
    ...
    <div class="p-6 bg-white rounded-lg shadow-lg">
      ...
      <DataTable :value="proyecto.recursos" size="small">
        <Column field="pivot.fecha_asignacion" header="FECHA ASIGNACIÓN"></Column>
  ...
  <div class="w-full mx-auto my-8 p-6 bg-white rounded-lg shadow-lg">
    ...
    {{recurso_proyecto}}
      <div class="flex flex-col gap-6">
        <div>
            <Select v-model="recurso_proyecto.recurso_id" :options="recursos" optionLabel="nombre" optionValue="id" placeholder="Seleccionar un Recurso" class="w-full md:w-56" fluid />
        </div>

        <div>
            <label for="cantidad_asignada" class="block font-bold mb-3">cantidad_asignada</label>
            <InputNumber id="cantidad_asignada" v-model="recurso_proyecto.cantidad_asignada" integeronly fluid />
        </div>
        <div>
            <label for="cantidad_asignada" class="block font-bold mb-3">Seleccionar Responsable</label>

            <Select v-model="recurso_proyecto.responsable_id" :options="usuarios" optionLabel="name" optionValue="id" placeholder="Seleccionar Responsable" class="w-full md:w-56" fluid />
        </div>
...
const asignarRecurso = async () => {
    ...
    const {data} = await proyectoService.asignarRecurso(route.params.id, recurso_proyecto.value);
